...
if(empty($res['status']) || $res['status']==0){
  header('Location: custodial1.php?res=nohtno');
}
else{
  require('header.php');
  ?>
      ...
      <div class="col-xs-12 col-sm-9">
        ...
        <div class="panel panel-info">
          ...
          <div class="panel-body">
            <form class="form-horizontal" role="form" action="custodial3.php" method="post">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-6">
                  <?php
					...
					$docs1 = $cgs_obj->getDocs();

					foreach($docs1 as $key => $value){
						if($key=="ssc" || $key=="cbsc" || $key=="icse"){
							echo "<input type='radio' name='docs1' value='".$value."'>".$value."<br/>" ;
						}
					}

					$docs = $cgs_obj->getDocs();
                    if(substr($res['htno'],4,1)=="5"){
                      foreach ($docs as $key => $value) {
                        if($key!=="inter" && $key!=="ssc" && $key!=="cbsc" && $key!=="icse"){
                          echo "<input type='checkbox' name='docs[]' value='".$key."' checked> ".$value."<br/>";
                        }
                      }
                    }
                    else{
